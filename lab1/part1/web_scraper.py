...
# 读取Excel文件
df = pd.read_csv('part1/data/Book_id.csv')
# 选择要读取的列
book_id_data = df['1046265'].tolist()
book_id_data.insert(0, '1046265')

df = pd.read_csv('part1/data/Movie_id.csv')
...
if __name__ == "__main__":
    ...
    #     movie_toExcel(movie_list, 'part1/data/movie.xlsx')
    #     delay = random.randint(0, 5)  # 随机间隔0-5s访问
    #     time.sleep(delay)
    ...
    for id in book_id_data:
        ...
        book_toExcel(book_list, 'part1/data/book.xlsx')
        delay = random.randint(0, 5)  # 随机间隔0-5s访问
        time.sleep(delay)
    print("错误id", wrong_id)
