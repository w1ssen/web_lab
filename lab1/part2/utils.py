...
def collate_fn(batch_data):
    ...
    for data, u_items_u, u_users_u, u_users_items_u, i_users_i in batch_data:
        ...
        if len(u_users_u) <= truncate_len:
            ...
            sample_index = random.sample(list(range(len(u_users_u))),
                                         truncate_len)
    ...
    u_user_item_pad = torch.zeros(
        [batch_size, u_users_maxlen, u_items_maxlen, 2], dtype=torch.long)
    ...
    return torch.LongTensor(uids), torch.LongTensor(iids), torch.FloatTensor(
        labels), u_item_pad, u_user_pad, u_user_item_pad, i_user_pad
