...
def zip_inverted_index():
    if os.path.exists('block_metadata.pkl'):  # 存在压缩的索引表后，不再构建压缩索引表
        return
    ...
    for i in range(len(movie_list_id)):
        ...
        if len(current_block) >= block_size:
            block_number += 1
            block_filename = f"block/block_{block_number}.pkl"  # 文件名示例
    ...
    with open('block_metadata.pkl', 'wb') as metadata_file:
        pickle.dump(block_metadata, metadata_file)


# 压缩索引检索
def search_in_zip(keyword):
    with open('block_metadata.pkl', 'rb') as metadata_file:
        block_metadata = pickle.load(metadata_file)

    for block_info in block_metadata:
        block_filename = f"block/block_{block_info['block_number']}.pkl"
